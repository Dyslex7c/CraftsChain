...
export default function CraftItemsSection() {
  ...
  useEffect(() => {
      ...
      if (isOwner && uriCheckDone && ecoCheckDone) {
        try {
          ...
          if (tokenURI) {
            ...
            setCraftItems(prev => [...prev, {
              id: currentTokenToCheck,
              name: metadata.name,
              image: imageUri.startsWith("bafkrei") ? `${IPFS_GATEWAY}${imageUri}` : imageUri,
    ...
    currentTokenIndex, tokenIds, ownerCheckDone, uriCheckDone, ecoCheckDone, 
    isOwner, tokenURI, currentTokenToCheck, isEcoFriendly
  ]);
    ...
    >
      ...
      {loading ? (
        <div className="p-8 text-center text-gray-500">
          Loading your craft items from the blockchain... {currentTokenIndex}/{tokenIds.length}
        </div>
      ) : craftItems.length === 0 ? (
        <div className="p-8 text-center text-gray-500">
